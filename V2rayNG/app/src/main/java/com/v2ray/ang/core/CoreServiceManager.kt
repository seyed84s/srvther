...
//        if (!result.status) error(result.errorMessage.ifBlank { "Failed to get V2Ray config" })
    ...
    private fun doStartCoreLoop(service: Service, vpnInterface: ParcelFileDescriptor?) {
        ...
        // Start Aether Engine as upstream proxy (chain mode)
        val aetherChainEnabled = MmkvManager.decodeSettingsBool(AppConfig.PREF_AETHER_CHAIN_ENABLED, true) // default ON
        if (aetherChainEnabled) {
            val mode = MmkvManager.decodeSettingsString(AppConfig.PREF_AETHER_CHAIN_MODE) ?: "masque"
            com.v2ray.ang.srvx.AetherEngine.start(service, mode)
            
            // Wait for Aether SOCKS5 to be ready (max 8 seconds)
            val ready = waitForAetherReady()
            if (!ready) {
                com.v2ray.ang.srvx.AetherEngine.stop()
                error("Aether Engine failed to start - SOCKS5 proxy not ready")
            }
            LogUtil.i(AppConfig.TAG, "StartCore-Manager: Aether Engine ready on port ${com.v2ray.ang.srvx.AetherEngine.SOCKS_PORT}")
        }
        ...
        while (System.currentTimeMillis() - start < timeoutMs) {
            try {
                java.net.Socket().use { socket ->
                    socket.connect(
                        java.net.InetSocketAddress("127.0.0.1", com.v2ray.ang.srvx.AetherEngine.SOCKS_PORT),
                        500
                    )
                    return true
                }
