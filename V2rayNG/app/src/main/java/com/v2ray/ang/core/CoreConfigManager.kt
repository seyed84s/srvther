...
object CoreConfigManager {
    ...
    private fun buildUnifiedConfig(configContext: CoreConfigContext): V2rayConfig {
        ...
        // Inject Aether chain if enabled
        val aetherChainEnabled = MmkvManager.decodeSettingsBool(AppConfig.PREF_AETHER_CHAIN_ENABLED, true)
        if (aetherChainEnabled) {
            injectAetherChain(v2rayConfig)
        }
    ...
    /**
     * Injects Aether Engine as an upstream SOCKS5 proxy for the primary outbound.
     * This creates a proxy chain: User → Xray (VLESS) → Aether SOCKS5 (:1819) → Internet
     */
    private fun injectAetherChain(v2rayConfig: V2rayConfig) {
        // Find the primary proxy outbound (first outbound, usually tagged "proxy")
        val primaryOutbound = v2rayConfig.outbounds.firstOrNull() ?: return
        
        // Skip if the primary outbound is already an Aether SOCKS5 profile
        // (i.e., it's already pointing to 127.0.0.1:1819)
        if (primaryOutbound.protocol == "socks" && primaryOutbound.tag == AppConfig.TAG_PROXY) {
            val settings = primaryOutbound.settings
            if (settings != null) {
                val serversJson = settings.servers
                if (serversJson != null && serversJson.any { 
                    it.address == "127.0.0.1" && it.port == com.v2ray.ang.srvx.AetherEngine.SOCKS_PORT 
                }) {
                    // Already an Aether profile, skip injection
                    return
                }
            }
        }
        
        // Create Aether upstream SOCKS5 outbound
        val aetherOutbound = V2rayConfig.OutboundBean(
            tag = "aether-upstream",
            protocol = "socks",
            settings = V2rayConfig.OutboundBean.OutSettingsBean(
                servers = listOf(
                    V2rayConfig.OutboundBean.OutSettingsBean.ServersBean(
                        address = "127.0.0.1",
                        port = com.v2ray.ang.srvx.AetherEngine.SOCKS_PORT,
                    )
                )
            ),
            streamSettings = null,
        ...
        // Add the Aether outbound to the config
        v2rayConfig.outbounds.add(aetherOutbound)
        
        // Set the primary outbound to chain through Aether using proxySettings
        primaryOutbound.proxySettings = V2rayConfig.OutboundBean.ProxySettingsBean(
            tag = "aether-upstream",
            transportLayer = true
        )
        
        LogUtil.i(AppConfig.TAG, "Injected Aether chain: ${primaryOutbound.tag} → aether-upstream (127.0.0.1:${com.v2ray.ang.srvx.AetherEngine.SOCKS_PORT})")
    }
